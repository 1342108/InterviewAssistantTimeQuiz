package com.example.android.interviewassistant.DbHelper;
...
import com.example.android.interviewassistant.Model.Question1;
import com.example.android.interviewassistant.Model.Ranking;
import com.example.android.interviewassistant.Common.QuestionLevel;
...
public class DbHelper extends SQLiteOpenHelper {
    ...
    //CRUD For Table
    public List<Question1> getAllQuestion() {
        List<Question1> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        try {
            c = db.rawQuery("SELECT * FROM Question1 ORDER BY Random()", null);
            ...
            do {
                ...
                Question1 question = new Question1(Id, Image, AnswerA, AnswerB, AnswerC, AnswerD, CorrectAnswer);
    ...
    public List<Question1> getQuestionMode(String mode) {
        List<Question1> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        int limit = 0;
        if (mode.equals(QuestionLevel.MODE.EASY.toString()))
            limit = 5;
        else if (mode.equals(QuestionLevel.MODE.MEDIUM.toString()))
            limit = 8;
        else if (mode.equals(QuestionLevel.MODE.HARD.toString()))
            limit = 10;
        else if (mode.equals(QuestionLevel.MODE.HARDEST.toString()))
            limit = 12;
        try {
            c = db.rawQuery(String.format("SELECT * FROM Question1 ORDER BY Random() LIMIT %d", limit), null);
            if (c == null) return null;
            c.moveToFirst();
            do {
                int Id = c.getInt(c.getColumnIndex("ID"));
